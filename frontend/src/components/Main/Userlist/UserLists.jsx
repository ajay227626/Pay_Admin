...
import { useNotification, useUserLists, useModal, generateRandomPassword, useSettings, dateTimeFoormatter } from "../../SettingsProvider/SettingsProvider";
import MessagePrompt from "../../MessagePrompt/MessagePromt";
            ...
            <div id="newUserListForm">
                ...
                <div id="contactInfo" className="tab-content">
                    ...
                    <div className="form-group">
                        ...
                        <input type="email" id="userListEmail" readOnly value={userList.email} name="contact_info.email" required placeholder="userList@example.com" />
                    </div>
                    <div className="form-group">
                ...
                <div id="security" className="tab-content">
                    <div className="form-group">
                        ...
                        <input type="password" id="userListPassword" readOnly value="********" name="contact_info.password" placeholder="Password" />
                    </div>
                    <div className="form-grid">
                        <div className="form-group">
                        ...
                        <div className="form-group">
                            <label htmlFor="userListOTP">Last Login</label>
                            <input id="userListOTP" readOnly value={userList.userListOtpNeeded} name="contact_info.last_login" />
                        </div>
                    </div>
                </div>
            </div>
...
const editableUserListModal = (userList) => {
    return (
        <div className="userList-modal-container">
            ...
            <div id="newUserListForm">
                ...
                <div id="contactInfo" className="tab-content">
                    ...
                    <div className="form-group">
                        <label htmlFor="userListEmail">Email Address <span className="required">*</span></label>
                        <input type="email" id="userListEmail" defaultValue={userList.email} onChange={(e) => setEmail(e.target.value?.trim())} name="contact_info.email" required placeholder="userList@example.com" />
                    </div>
                    <div className="form-group">
...
function UserListsList({ userListData, setAllCheckboxSelected, switchTab, setUserListID, isAdmin, permission, editAddAdminCheck, selectedListCheckboxCount, setSelectedListCheckboxCount }) {
    ...
    const editUserListModal = (userList) => {
        setUserListFirstName(userList.userListFirstName);
        setUserListLastName(userList.userListLastName);
        setUserListRole(userList.userListRole);
        setUserListDepartment(userList.userListDepartment);
        setUserListAddress(userList.userListAddress);
        setUserListPhone(userList.userListPhone);
        setUserListSecondaryPhone(userList.userListSecondaryPhone);
        setUserListEmail(userList.email);
        setUserListPersonalEmail(userList.userListPersonalEmail);
        setUserListAllowedDevices(userList.userListAllowedDevices);
        setUserListAllowedDevicesCount(userList.userListAllowedDevicesCount);
        setUserListAllowLogin(userList.userListAllowLogin);
        setUserListPassword(userList.userListPassword);
        setUserListStatus(userList.userListStatus);
        setUserListOtpNeeded(userList.userListOtpNeeded);
        return (
            <div className="userList-modal-container">
                <div className="modal-tabs">
                    <button className="tab-button active" onClick={(e) => switchTab(e, 'userInfo')}>
    ...
    const handleRecordListCheckboxChange = (e) => {
        const isChecked = e.target.checked;
        const checkboxCount = document.querySelectorAll('.userList-list-row input[type="checkbox"]:checked').length;
        setSelectedListCheckboxCount(checkboxCount);
        console.log('Checkbox Count:', checkboxCount);
        setAllCheckboxSelected(isChecked);
    }
    return (
        <>
                ...
                <table className="userList-list">
                    <thead className="userList-list-header">
                        <tr>
                            ...
                            {isAdmin && <th className="user-field-header show">Password</th>}
                            ...
                            userListData.map((userList, index) =>
                                (index < rowsPerPage * currentPage && index >= rowsPerPage * (currentPage - 1)) ? (
                                    <tr className="userList-list-row" key={index}>
                                        <td style={{ alignContent: 'end', padding: 'var(--padding)' }}><input type="checkbox" onClick={handleRecordListCheckboxChange} /></td>
                                        ...
                                        <td className="user-field-data show" style={ userList.email ? { textTransform: 'lowercase' } : { textAlign: 'center' }}>{userList.email || '-'}</td>
                                        ...
                                        <td className="user-field-data show" style={{ textAlign: 'center' }}>{dateTimeFoormatter(userList.lastLogin, 'short', 'short', 'numeric', 'numeric', 'numeric', 'numeric', true, true, 'IST', 'en-IN') || '-'}</td>
                                        <td className="user-field-data show" style={{ textAlign: 'center' }}>{dateTimeFoormatter(userList.timestamp, 'short', 'short', 'numeric', 'numeric', 'numeric', 'numeric', true, true, 'IST', 'en-IN') || '-'}</td>
                                        <td className="user-field-data show" style={{ textAlign: 'center' }}>{userList.parentUser || '-'}</td>
                                        <td className="user-field-data show" style={{ textAlign: 'center', display: editAddAdminCheck ? 'flex' : '', gap: '5px' }}>
...
function UserListsGrid({ userListData, isAdmin, permission, editAddAdminCheck, selectedGridCheckboxCount, setSelectedGridCheckboxCount }) {
    ...
    const openViewModal = (e, userList) => {
        e.stopPropagation();
        userMenuAction(e);
        showNotification( 'User List Details', 'positive' );  
        showModal(
            'View User Details',
            '',
            viewUserListModal(userList),
                ...
                <div id="newUserListForm">
                    ...
                    <div id="userInfo" className="tab-content active">
                        <div className="form-grid">
                            <div className="form-group">
                                <label htmlFor="userListFirstName">First Name <span className="required">*</span></label>
                                <input type="text" id="userListFirstName" defaultValue={userList.userListFirstName} onChange={(e) => { setUserListFirstName(e.target.value?.trim()) }} name="firstName" placeholder="First name" />
                            </div>
                            <div className="form-group">
                                <label htmlFor="userListLastName">Last Name <span className="required">*</span></label>
                                <input type="text" id="userListLastName" defaultValue={userList.userListLastName} onChange={(e) => { setUserListLastName(e.target.value?.trim()) }} name="lastName" placeholder="Last name" />
                            </div>
                        </div>
                        <div className="form-grid">
                            <div className="form-group">
    ...
    const openEditModal = (e, userList) => {
        showNotification( 'Edit User List', 'positive' );
        e.stopPropagation();
        userMenuAction(e);
        setType('edit');
        showModal(
    ...
    const openDeleteModal = (e, userList) => {
        showNotification( 'Delete User List', 'negative' );
        e.stopPropagation();
        userMenuAction(e);
    }
    const openResetPasswordModal = (e, userList) => {
        showNotification( 'Reset User Password', 'positive' );
        e.stopPropagation();
        userMenuAction(e);
    }
    const userMenuAction = (event) => {
        event.stopPropagation();
        const userAction = event.currentTarget.closest('.action-ellipsis');
        const userGridOptions = document.querySelector('.user-grid-options');
        ...
        userGridOptions.style.zIndex = 10;
        userGridOptions.classList.toggle('active');
        const otherActions = document.querySelectorAll('.user-grid-options.active');
        otherActions.forEach(action => { if (action !== userGridOptions) action.classList.remove('active'); });
        const otherIcons = document.querySelectorAll('.btn-action.active');
        document.addEventListener('click', (e) => {
            if (!userAction.contains(e.target)) userGridOptions.classList.remove('active');
        }, { once: true });
    };
    const handleRecordGridCheckboxChange = (e) => {
        const isChecked = e.target.checked;
        const checkboxCount = document.querySelectorAll('.userList-grid input[type="checkbox"]:checked').length;
        setSelectedGridCheckboxCount(checkboxCount);
        console.log('Grid Checkbox Count:', checkboxCount);
        // setAllCheckboxSelected(isChecked);
    };
    return (
        <div className="userList-grid-container">
            {
                userListData.map((userList, index) => (
                    <div className="userList-grid" key={index}>
                        <div className="action-ellipsis">
                            <button className="btn btn-primary gridViewOptions" onClick={userMenuAction}><i className="fas fa-ellipsis-v"></i></button>
                            <div className="user-grid-options">
                                <div className="grid-menu-arrow" />
                                <div className="grid-menu-items">
                                    <div className="grid-menu-item" onClick={(e) => { openViewModal(e, userList) }}>
                                        <i className="fas fa-eye"></i> View Details
                                    </div>
                                    {editAddAdminCheck && <div className="grid-menu-item" onClick={(e) => { openEditModal(e, userList) }}>
                                        <i className="fas fa-edit"></i> Edit User
                                    </div>}
                                    {editAddAdminCheck && <div className="grid-menu-item" onClick={(e) => { openDeleteModal(e, userList) }}>
                                        <i className="fas fa-trash"></i> Delete User
                                    </div>}
                                    {editAddAdminCheck && <div className="grid-menu-item" onClick={(e) => { openResetPasswordModal(e, userList) }}>
                                        <i className="fas fa-key"></i> Reset Password
                                    </div>}
                                </div>
                            </div>
                        </div>
                        {userList.userListFirstName && <div className="userList-grid-name" title={'Full Name'}><input type="checkbox" className="user-grid-checkbox" onClick={handleRecordGridCheckboxChange} /><i className="fas fa-user facolor"></i>{userList.userListFirstName} {userList.userListLastName}</div>}
...
function UserLists({ status, role = "Minion" }) {
    const isAdmin = role === "Minion";
    const { userSetting, permissionSettings } = useSettings();
    const permission = permissionSettings['userlists-page']?.[role];
    const [ userTableHeaders, setUserTableHeaders ] = React.useState([]);
    const [ colData, setColData ] = React.useState('');
    const [ currentView, setCurrentView ] = React.useState(localStorage.getItem('userListView') || 'list');
    ...
    const [ selectedListCheckboxCount, setSelectedListCheckboxCount ] = React.useState(0);
    const [ selectedGridCheckboxCount, setSelectedGridCheckboxCount ] = React.useState(0);
    const [ loading, setLoading ] = React.useState(true);
    const [ selectedRows, setSelectedRows ] = React.useState([]);
    const [ visibleColumns, setVisibleColumns ] = React.useState(() => {
        const saved = localStorage.getItem("userColumnVisibilityList");
        return saved ? JSON.parse(saved) : new Array(19).fill(true);
    });
    const { showModal, closeModal } = useModal();
    const {
        userListFirstName,
                        ...
                        <div className="form-grid">
                            <div className="form-group">
                                ...
                                <select id="userListRole" name="role" defaultValue='Selected UserList Role' onChange={(e) => { setUserListRole(e.target.value), handleUserListEntry(e) }} required>
                                    <option value="">Select User Role</option>
                                    {userSetting.userRoles.map((role, index) => ( <option key={index} value={role}>{role}</option> ))}
                                </select>
                            </div>
                            <div className="form-group">
                                <label htmlFor="userListDepartment">Department</label>
    ...
    const handleSelectAll = () => {
        const newCheckedState = !allCheckboxSelected;
        setAllCheckboxSelected(newCheckedState);
        let checkCounter = 0, selectedIndexes = [];
        const checkboxes = currentView === 'list' ? document.querySelectorAll('.userList-list-row > td > input[type=checkbox]') : document.querySelectorAll('.userList-grid-name > input[type=checkbox]');
        ...
        console.log(newCheckedState ? selectedIndexes : []);
        setSelectedRows(newCheckedState ? selectedIndexes : []);
        setSelectedListCheckboxCount(checkCounter);
        setSelectedGridCheckboxCount(checkCounter);
    }
    ...
    const handleColumnSelect = (e) => {
        ...
        localStorage.setItem('userColumnVisibilityList', JSON.stringify(updatedVisibility));
    };
    const editAddAdminCheck = isAdmin || permission === 'add' || permission === 'edit';
    useEffect(() => {
        const handleCheckboxChange = () => {
            const rows = document.querySelectorAll('.userList-list-row');
            const checkboxes = currentView === 'list' ? document.querySelectorAll('.userList-list-row > td > input[type=checkbox]') : document.querySelectorAll('.userList-grid-name > input[type=checkbox]');
            const selectedIndexes = [];
            checkboxes.forEach((checkbox, index) => { if (checkbox.checked) selectedIndexes.push(index + 1); });
            setSelectedRows(selectedIndexes);
            setSelectedListCheckboxCount(currentView === 'list' ? selectedIndexes.length : 0);
            setSelectedGridCheckboxCount(currentView === 'grid' ? selectedIndexes.length : 0);
            console.log('Selected Indexes (manual):', selectedIndexes);
        };
        document.addEventListener('change', handleCheckboxChange);
        return () => { document.removeEventListener('change', handleCheckboxChange); };
    }, [currentView]);
    const handleDeleteSelections = () => {
        const selectedCheckboxes = currentView === 'list' ? document.querySelectorAll('.userList-list-row > td > input[type=checkbox]:checked') : document.querySelectorAll('.userList-grid-name > input[type=checkbox]:checked');
        showNotification("Delete Selected Users", "negative");
    };
    const openMessageModal = () => {
        showNotification("Send Notification to Selected Users", "positive");
        showModal(
            'Send Notification',
            '',
            <MessagePrompt />,
        ...
        <section id="userlists-page" className={`page ${status}`}>
            ...
            <div className="page-filters">
                ...
                <div className="filter-group">
                    ...
                    <button className="btn btn-primary" id="selectAllBtn" onClick={handleSelectAll} title={allCheckboxSelected ? 'Unselect All' : 'Select All'}>{allCheckboxSelected ? <i className="fas fa-times" style={{ color: 'var(--danger-color)' }}></i> : <i className="fas fa-check-double"></i> }</button>
                    <button className="btn btn-primary" id="deleteSelectedBtn" onClick={handleDeleteSelections} title="Delete Selected" disabled={!selectedRows.length}><i className="fas fa-trash"></i></button>
                    <button className="btn btn-primary" id="sendNotificationBtn" onClick={openMessageModal} title="Send Notification" disabled={!selectedRows.length}><i className="fas fa-paper-plane"></i></button>
                </div>
                <div className="action-side">
                    ...
                    <button className="btn btn-primary" id="reloadData" style={{ borderRadius: '50%', padding: '0.5rem'}} onClick={() => {
                        fetchUserLists();
                        handleReset();
                        showNotification("User Lists reloaded", "positive");
                    }}><i className="fas fa-sync"></i></button>
                    <div className="view-group">
                        <button className={`btn btn-primary ${currentView === 'list' ? 'active' : ''}`} id="listViewBtn" onClick={() => { handleView(); setAllCheckboxSelected(false); }}>
                            <i className="fas fa-list"></i>
                        </button>
                        <button className={`btn btn-primary ${currentView === 'grid' ? 'active' : ''}`} id="gridViewBtn" onClick={() => { handleView(); setAllCheckboxSelected(false); }}>
            ...
            <div className="content-area">
                {currentView === 'list' ?
                    <UserListsList
                        ...
                        editAddAdminCheck={editAddAdminCheck}
                        selectedListCheckboxCount={selectedListCheckboxCount}
                        setSelectedListCheckboxCount={setSelectedListCheckboxCount}
                    /> :
                    <UserListsGrid
                        userListData={filteredData || []}
                        setAllCheckboxSelected={setAllCheckboxSelected}
                        ...
                        permission={permission}
                        editAddAdminCheck={editAddAdminCheck}
                        selectedGridCheckboxCount={selectedGridCheckboxCount}
                        setSelectedGridCheckboxCount={setSelectedGridCheckboxCount}
                    />}
            </div>
        </section>
    );
